...
class SquareTests (unittest.TestCase):
    ...
    def test_SizeUpdatesWidthAndHeight(self):
        ...
        s = Square(3)
        assertEqual(s.width, 3)
        assertEqual(s.height, 3)
        s.size = 100
        assertEqual(s.width, 100)
        assertEqual(s.height, 100)
    ...
    def test_UpdateAttributes(self):
        ...
        s = Square(3, 10, 4, 'square')
        s.update('changed')
        assertEqual(s.id, 'changed')
        s.update('back', 5)
        assertEqual(s.id, 'back')
        assertEqual(s.size, 5)
        s.update('another', 20, size=30)
        assertEqual(s.size, 20)
        s.update('id', 1, 2, 3)
        assertEqual(s.x, 2)
        assertEqual(s.y, 3)
        s.update(y=50)
        ...
        assertEqual(s.id, 'id2')
        assertEqual(s.size, 15)
        assertEqual(s.x, 0)
        assertEqual(s.y, 1)
